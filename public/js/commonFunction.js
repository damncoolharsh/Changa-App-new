var blockUImessage = $(
    '<div class="blockui-message bg-info"><div class="uil-ring-css"  style="transform:scale(0.38);"><div></div></div> <span class="display-block">Just a sec &hellip; processing request</span></div>'
);
...
function postMultipartAjax(
    ...
    if (typeof args !== "undefined") {
        ...
        if (args.length > 1) loading_msg = args[1];
        if (args.length > 2) success_icon = args[2];
        if (args.length > 3) loading_icon = args[3];
        if (args.length > 4) error_icon = args[4];
    }
    ...
    blockUImessage
        ...
        .addClass("bg-info")
        .html(
            '<div class=""  style="transform:scale(0.38);"><div></div></div><i class="' +
                loading_icon +
                '"></i> <span class="display-block">' +
                loading_msg +
                "</span>"
        );
    blockUI(status);
        ...
        success: function (data, textStatus, XMLHttpRequest) {
            ...
            if (data.error.length > 0) {
                if (!data.swal) {
                    blockUImessage
                        .removeClass("bg-info")
                        .addClass("bg-" + (data.valid ? "success" : "danger"))
                        .html(
                            '<i class="' +
                                (data.valid ? success_icon : error_icon) +
                                '"></i> <span class="display-block">' +
                                data.error +
                                '<!--<button type="button" class="btn btn-default">OK</button>--></span>'
                        );
                } else {
                    unblockUI(status);
            ...
            if (showExp) {
                blockUImessage
                    ...
                    .html(
                        '<i class="' +
                            error_icon +
                            '"></i> <span class="display-block">Error while contacting server: ' +
                            errorThrown +
                            "</span>"
                    );
                unblockUI(status, 500);
...
function unblockUI(block, delay) {
    ...
    blockUImessage = $(
        '<div class="blockui-message bg-info"><i class="fa fa-spinner fa-spin"></i> <span class="display-block">Just a sec &hellip; processing request</span></div>'
    );
}
                    ...
                    if (data.validations[prop][0] != undefined) {
                        if (typeof data.validations[prop][key] == "string") {
                            ...
                            for (var assosKey in data.validations[prop][key]) {
                                for (errorKey in data.validations[prop][key][
                                    assosKey
                                ]) {
                                    var propId =
                                        "." + prop + "-" + key + "-" + assosKey;
